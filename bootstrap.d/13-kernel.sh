...
mkdir -p "${R}/etc/kernel/postinst.d"
mkdir -p "${R}/etc/kernel/preinst.d"
install_exec files/etc/kernel/postinst.d/process-vmlinuz "${R}/etc/kernel/postinst.d"
install_exec files/etc/kernel/postinst.d/update-config "${R}/etc/kernel/postinst.d"
install_exec files/etc/kernel/preinst.d/preprocess-vmlinuz "${R}/etc/kernel/preinst.d"
install_readonly files/etc/kernel-img.conf "${R}/etc"

if [ ! -d kernel-packages ]; then

	if [  -d "${RPI_FIRMWARE_DIR}" ] && [ "$BUILD_KERNEL" = false ]; then
	...
	if [ "$BUILD_KERNEL" = true ] ; then
		...
		if [ "$KERNEL_REMOVESRC" = true ] ; then
		  ...
		  # Kernel installation
		  install_deb linux-image-"${COLLABORA_KERNEL}" raspberrypi-bootloader-nokernel
	...
	# Install Kernel packages
		for package in kernel-packages/*.deb ; do
			cp $package ${R}/tmp
